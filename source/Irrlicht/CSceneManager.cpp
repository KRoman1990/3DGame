...
ISceneNodeAnimator* CSceneManager::createFlyCircleAnimator(
		const core::vector3df& center, f32 radius, f32 m_speed,
		const core::vector3df& direction,
		f32 startPosition,
		f32 radiusEllipsoid)
{
	const f32 orbitDurationMs = (core::DEGTORAD * 360.f) / m_speed;
	...
	ISceneNodeAnimator* anim = new CSceneNodeAnimatorFlyCircle(
			...
			radius, m_speed, direction,radiusEllipsoid);
	return anim;
}
...
ISceneNodeAnimator* CSceneManager::createFollowSplineAnimator(s32 startTime,
	...
	f32 m_speed, f32 tightness, bool loop, bool pingpong)
{
	ISceneNodeAnimator* a = new CSceneNodeAnimatorFollowSpline(startTime, points,
		m_speed, tightness, loop, pingpong);
	return a;
}
